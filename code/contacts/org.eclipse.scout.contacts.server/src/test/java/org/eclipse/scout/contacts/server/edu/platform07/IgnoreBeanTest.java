...
public class IgnoreBeanTest {
  ...
  @Test
  public void test() {
    List<String> birdNames = BEANS.all(IBird.class)
        .stream()
        .map(b -> b.getClass().getSimpleName())
        .collect(Collectors.toList());
    List<String> dragonNames = BEANS.all(IDragon.class)
        .stream()
        .map(d -> d.getClass().getSimpleName())
        .collect(Collectors.toList());

    assertThat(birdNames, hasSize(2));
    assertEquals(birdNames, Arrays.asList(Falkor.class.getSimpleName(), Sparrow.class.getSimpleName()));

    assertThat(dragonNames, hasSize(1));
    assertEquals(dragonNames, Arrays.asList(Falkor.class.getSimpleName()));
  }
  ...
  public void fishTest() {
    List<String> fishNames = BEANS.all(IFish.class)
        .stream()
        .map(f -> f.getClass()
            .getSimpleName())
        .collect(Collectors.toList());

    assertThat(fishNames, hasSize(2));
    assertEquals(fishNames, Arrays.asList(Tuna.class.getSimpleName(), Whale.class.getSimpleName()));
