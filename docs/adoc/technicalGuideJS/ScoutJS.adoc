...
Creating the form is now possible using the following code (assuming your namespace is called 'example').
[source,javascript]
----
...
----
var field = scout.create('StringField', {
  parent: scout.App.get().sessions[0].desktop,
  labelVisible: false
});
...
== Styling
Beside JavaScript for business logic and for the models, every Scout JS app probably needs some CSS code at some point of time. If you are writing custom widgets, you need it for sure. But also if you are just using the given widgets you might have the need to adjust the look here and there.
...
The creation of these files won't make them loaded automatically, you have to register them in your `index` file. This file should be put into your `src/main/js` folder and named `index.less`. Just include your new files using the LESS `@import` keyword.
...
=== scout:script
Loads the referenced JavaScript macro and starts the JavaScript build for that file. Outputs a `script` tag in the HTML document which references the generated .js file. "bsicrm-all-macro.js" will become "bsicrm-all.js" in the parsed HTML document. Depending on Scout's runtime properties the filename will be augmented: if caching is enabled an additional fingerprint is added to the filename. If minifying is enabled the suffix "-min" is appended to the filename.

Example:

  <scout:script src="res/bsicrm-all-macro.js" />

Output:

  <script src="res/bsicrm-all-98aea5b3.min.js"></script>

=== scout:stylesheet
Loads the referenced LESS macro and starts the CSS build with LESS for that file. Outputs a `link` tag in the HTML document which references the generated .css file. "bsicrm-all-macro.less" will become "bsicrm-css.js" in the parsed HTML document. Depending on Scout's runtime properties the filename will be augmented: if caching is enabled an additional fingerprint is added to the filename. If minifying is enabled the suffix "-min" is appended to the filename.

Example:

  <scout:stylesheet src="res/bsicrm-all-macro.less" />

Output:

  <link rel="stylesheet" type="text/css" href="res/bsicrm-all-98aea5b3.min.css">
...
==== Bundling
The main purpose of Webpack is to bundle the many small source files into one or a few larger JavaScript or Less files which are included in the HTML files as `<script>` resp. `<style>` tags and therefore loaded by the browser. There are a lot of different strategies and opinions on how to bundle correctly. If you have a strong opinion about it you can use your own bundling strategy. If not you should be fine with the Scout defaults which are as follows:

There are 4 JavaScript bundles:

- `eclipse-scout.js`: contains all official Scout code (from modules with scope @eclipse-scout).
- `jquery.js`: contains the jQuery code.
- `vendors.js`: contains all other third party code (libraries in the node_modules folder). This file is only created if you have other third party code. This is not the case for a newly created Scout app so your index.html may not include it yet.
- `your-app.js`: contains all the code from your app.

There are some reasons why we chose this strategy:

- Optimal caching: the bundles can be reused by different HTML files, e.g. `eclipse-scout.js` and `jquery.js` can be used by `index.html`, `login.html` and `logout.html`. The browser only has to load the files once and can reuse them for the other HTML pages.
- Keep it simple: we think that this approach is simple to understand.
- Security: the login page should only load the code it needs to display the login page and should not reveal the application code. That is why `login.html` includes its own entry point (`login.js`) and does not use `your-app.js`.
...
==== Building
Before you can open your application in the browser, you need to build it. The build takes all your source code and resources and creates the artifacts needed for the browser according to your <<Webpack Configuration>>. Once the build is complete all the produced artifacts are put in the `dist` folder.

The `dist` folder contains three sub folders:
...
. clean: true, to clean the dist folder before each build. Default is false if watcher is enabled (build:dev:watch), otherwise true.
...
* Copy _/dist/prod_ and _/dist/res_ to the folder _/target/classes_
...
* Check your index.html in the browser. Each referenced script or CSS file should have a fingerprint, example: `eclipse-scout-2c6053b2fdf5b816fae5.min.js`.
...
NOTE: If the command takes very long and prints a lot of errors, you may have to ignore the `dist` folder, see <<ESLint Ignore>>.
...
==== ESLint Ignore
Similar to `.gitignore`, you can create a file called `.eslintignore` to exclude specific files or directories from the analysis. Because analyzing the build output probably does not make any sense, we recommend to at least ignore the `dist` folder. The only thing you need to do is to create that file and add a line with the word `dist`.
